<script>
  ...
  // we don't want the store to persist the hash when we navigate to and from
  $activeHash = "";

  $: if (ready && !$isScrolling) history.replaceStateNative({}, null, `/docs#${$activeHash}`);
</script>

<LiveAnchor bind:activeHash={$activeHash} let:anchors offset={96 + 72} anchorOffset="-{96 + 72}px">
  <div class="copy">
    {#if true}
                    ...
                    {#each topic.children.filter(noExample) as subject}
                      {#if subject.name !== "index"}
                        <h3 class="subject-header">
                          <Anchor id="{category.name}/{topic.name}/{subject.name}" />
                          <a href="#{category.name}/{topic.name}/{subject.name}">
                            {subject.name}
                          </a>
                        </h3>
                      {/if}
